...
export function MarketTicker() {
    ...
    const groupedStocks = useMemo(() => {
        ...
        stocks.forEach(stock => {
            ...
            if (groups[upperExchange]) {
                groups[upperExchange].push(stock);
            }
        });

        // Take the first stock from each exchange group
        const result = [];
        result.push(groups.NSE[0] || { symbol: 'RELIANCE', exchange: 'NSE', price: 0, change: 0 });
        result.push(groups.BSE[0] || { symbol: 'SENSEX', exchange: 'BSE', price: 0, change: 0 });
        result.push(groups.MCX[0] || { symbol: 'CRUDEOIL', exchange: 'MCX', price: 0, change: 0 });

        return result;
    }, [stocks]);

    // Only render if we have stocks
    if (groupedStocks.every(stock => !stock)) return null;
                ...
                <div className="flex items-center gap-4 bg-slate-800/70 px-4 py-1.5 rounded-lg border border-slate-600/60 shadow-sm">
                    ...
                    <div className="flex flex-col items-end">
                        ...
                        <div className={`flex items-center gap-0.5 text-[9px] font-bold ${parseFloat(groupedStocks[0]?.change || 0) >= 0 ? 'text-green-300' : 'text-red-300'
                            ...
                            {parseFloat(groupedStocks[0]?.change || 0) >= 0 ? '+' : ''}{groupedStocks[0]?.change || 0}%
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="flex items-center gap-4 bg-slate-800/70 px-4 py-1.5 rounded-lg border border-slate-600/60 shadow-sm">
                    ...
                    <div className="flex flex-col items-end">
                        ...
                        <div className={`flex items-center gap-0.5 text-[9px] font-bold ${parseFloat(groupedStocks[1]?.change || 0) >= 0 ? 'text-green-300' : 'text-red-300'
                            ...
                            {parseFloat(groupedStocks[1]?.change || 0) >= 0 ? '+' : ''}{groupedStocks[1]?.change || 0}%
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="flex items-center gap-4 bg-slate-800/70 px-4 py-1.5 rounded-lg border border-slate-600/60 shadow-sm">
                    ...
                    <div className="flex flex-col items-end">
                        ...
                        <div className={`flex items-center gap-0.5 text-[9px] font-bold ${parseFloat(groupedStocks[2]?.change || 0) >= 0 ? 'text-green-300' : 'text-red-300'
                            ...
                            {parseFloat(groupedStocks[2]?.change || 0) >= 0 ? '+' : ''}{groupedStocks[2]?.change || 0}%
                        </div>
                    </div>
                </div>
            </div>
