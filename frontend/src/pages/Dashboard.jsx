...
export function Dashboard() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            {/* Top Grid: Metrics & Intelligence */}
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                {/* Metrics Section (Left) - Spans 8 cols */}
                <div className="md:col-span-8 flex flex-col gap-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {metrics.map((metric, index) => (
                            <Link to={metric.link} key={index} className="block group">
                                <Card className="p-6 h-full flex flex-col justify-between border-slate-100 hover:border-indigo-100 hover:shadow-xl transition-all duration-300 bg-white group-hover:-translate-y-1">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className={`p-3 rounded-2xl ${metric.color.replace('text-', 'bg-')}/10 ${metric.color}`}>
                                            <metric.icon size={22} />
                                        </div>
                                        {metric.action && <div>{metric.action}</div>}
                                    </div>
                                    <div>
                                        <h3 className="text-slate-500 font-bold text-xs uppercase tracking-widest mb-1">{metric.label}</h3>
                                        <div className="flex items-baseline gap-2">
                                            <span className="text-2xl font-black text-slate-900 leading-none">{metric.value}</span>
                                        </div>
                                        <p className={`text-[10px] font-bold mt-2 ${metric.change.includes('+') ? 'text-green-600' : 'text-slate-400'}`}>
                                            {metric.change}
                                        </p>
                                    </div>
                                </Card>
                    ...
                    {/* Chart Section */}
                    <Card className="flex-1 p-8 border-slate-100 shadow-sm bg-white">
                        <div className="flex items-center justify-between mb-8">
                            {/* ... Chart Header ... */}
                            <h3 className="font-heading font-black text-slate-900 text-lg">Portfolio Performance</h3>
                            <div className="flex bg-slate-50 p-1 rounded-xl">
                                {['Today', 'This Week', 'This Year'].map((p) => (
                                    <button
                                        key={p}
                                        onClick={() => setPeriod(p)}
                                        className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${period === p ? 'bg-white shadow-sm text-primary' : 'text-slate-400 hover:text-slate-600'}`}
                                    >
                                        {p}
                                    </button>
                                ))}
                ...
                {/* Right Column: AI & Top Movers - Spans 4 cols */}
            ...
            {/* ... (Keep existing Holdings Table logic if present below this block) ... */}
