...
async def smoke_test():
    ...
    try:
        ...
        if result['status'] in expected_statuses:
            print(f"\n[SUCCESS] Smoke Test PASSED: Workflow reached status '{result['status']}'")
        else:
            print(f"\n[FAILED] Smoke Test FAILED: Unexpected status '{result['status']}'")
        ...
        traceback.print_exc()
        print(f"\n[FAILED] Smoke Test FAILED with Exception: {e}")
