#!/usr/bin/env python3
"""
Local seed script for StockSteward AI development environment
This script will create and seed a local SQLite database
"""
...
def seed_local_db():
    print("Recreating local SQLite database tables...")
    ...
    try:
        ...
        db.execute(text("DELETE FROM strategies"))
        db.execute(text("DELETE FROM holdings"))
        db.execute(text("DELETE FROM watchlist_items"))
        ...
        import os
        if os.path.exists('./stocksteward_local.db'):
            size = os.path.getsize('./stocksteward_local.db')
            print(f"Database file created: stocksteward_local.db ({size} bytes)")
        else:
