...
class AIFilterEngine(AIFilterEngineInterface):
    ...
    async def analyze_market_sentiment(self, news_data: List[Dict[str, Any]], social_data: List[Dict[str, Any]] = None, symbols: List[str] = None) -> Dict[str, Any]:
        """Analyze market sentiment from news and social media using deep FinBERT sequence classification"""
        try:
            social_data = social_data or []
            
            # Execute full FinBERT market corpus classification on news and social streams
            macro_result = finbert_engine.analyze_market_corpus(news_data, social_data)
            
            news_sentiment_score = macro_result["news_sentiment"]
            social_sentiment_score = macro_result["social_sentiment"]
            
            # If symbols specified, blend with historical database social sentiment scores
            if symbols:
                db_scores = await self._get_social_sentiment_from_db(symbols)
                if db_scores:
                    db_avg = sum(db_scores) / len(db_scores)
                    if social_data:
                        social_sentiment_score = (social_sentiment_score + db_avg) / 2.0
                    else:
                        social_sentiment_score = db_avg
                        
            # Recalculate combined macro sentiment
            if news_data and (social_data or symbols):
                normalized_sentiment = (news_sentiment_score * 0.6) + (social_sentiment_score * 0.4)
            elif news_data:
                normalized_sentiment = news_sentiment_score
            elif social_data or symbols:
                normalized_sentiment = social_sentiment_score
            else:
            ...
            sentiment_analysis = {
                "overall_sentiment": round(normalized_sentiment, 4),
                "news_sentiment": round(news_sentiment_score, 4),
                "social_sentiment": round(social_sentiment_score, 4),
                "confidence": 0.85,  # High confidence via FinBERT sequence classification
                "market_regime": macro_result.get("market_regime", "NEUTRAL"),
                "distribution": macro_result.get("distribution", {}),
                "top_movers": macro_result.get("top_market_movers", []),
                "model_engine": macro_result.get("model_engine", "ProsusAI/finbert"),
                "timestamp": datetime.now().isoformat()
            }

            logger.info(f"Completed market sentiment analysis via FinBERT: score={normalized_sentiment:.3f}")
